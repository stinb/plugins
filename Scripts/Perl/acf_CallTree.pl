#
# Synopsis: Create Call graphics files
#
# Language: C/C++, Ada, Fortran
#
# Usage:
sub usage ($)
{
    return << "END_USAGE";
${ \( shift @_ ) }
Usage: $0 -db database [-name pattern]
...
#  full of png files of the Call trees of all things with call trees
#  in the database that match the optional name pattern.
#
#  Ada, C or FORTRAN.
#
#  Designed as a GUI base script. 'CallTrees' directory is relative to the CWD
...
#  > Engine_oil_Call.f77
#  > should generate Engine_oil_Call.png or Engine_oil_Call.png
...
print "making ".$ent->longname." - ".$file."\n";
    my $status = $ent->draw("Calls",$file,"Level=5;Name=Fullname");
    if ( !$status ) {
...
sub makepngName($)
{
    ...
    my $name = ($file?$filename."-":"").$ent->longname() . ".png";
