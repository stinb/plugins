...
#
# Synopsis: Reports all global variables and any references to them.
#
...
sub usage ($)
{
    return << "END_USAGE";
${ \( shift @_ ) }
Usage: global_xref.pl -db database [-out file]
...
# Description:
#   Reports all global variables and any references to them.
#
# Ada:
...
# 
#  07-Aug-2001 DLL 
#  08-Sep-2001 DLL  Remove "inc" tests that are no longer necessary
#
use Understand;
use Getopt::Long;
use strict;
...
if ( $language =~ "Fortran" ) {
    fortranGetBlockVariables();
    fortranGetGlobalsInModules();
}
...
sub fortranGetBlockVariables()
{
    my (@ents)=$db->ents("Fortran Block Variable");
    foreach my $ent (@ents) {
	push(@globals,  $ent);
    }
}

sub fortranGetGlobalsInModules()
{
    # get all ents with define refs "Fortran Module"
    my (@ents)=$db->ents("Fortran Module");
    foreach my $ent (@ents) {
	my @refs=$ent->refs("define ~private","variable");
	foreach my $ref (@refs) {
	    push( @globals,  $ref->ent() );
	}
    }
}
