// Custom: multiple condition variables
...
int main()
{
    ...
    if ((ret = pthread_cond_init(&cv1, NULL)) != 0)
    {
        /* handle error */
    }
...
void *waiter1()
{
    ...
    while (count1 < COUNT_LIMIT)
    {
        ...
        if ((ret = pthread_cond_wait(&cv1, &mutex1)) != 0)
        {
            /* Handle error */
        }
...
void *signaler()
{
    ...
    while ((count1 < COUNT_LIMIT) || (count2 < COUNT_LIMIT))
    {
        sleep(1);
        printf("signaling\n");
        if ((ret = pthread_cond_signal(&cv1)) != 0)
        {
            /* Handle error */
        }
        if ((ret = pthread_cond_signal(&cv2)) != 0)
        {
            /* Handle error */
        }
    }
