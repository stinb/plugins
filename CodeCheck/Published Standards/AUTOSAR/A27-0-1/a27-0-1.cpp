...
void F1(const char* name) // name restricted to 256 or fewer characters
{
  ...
  len,
  format,
  name); // UndCC_Violation - no additional check for overflows
...
{
  static const char format[] = "Name: %s .";
  fprintf(stderr, format, name); // UndCC_Valid - untrusted input passed as one
  // of the variadic arguments, not as part of
  // vulnerable format string
}
void F3(const std::string& name)
{
  // UndCC_Valid, untrusted input not passed
