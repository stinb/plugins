...
void F1(T const& obj) noexcept(false)
{
    static_assert(
            ...
            "Given template type is not copy constructible."); // UndCC_Valid
}
template <typename T>
class C
{
...
{
    public:
        D() = default; // UndCC_Violation
        D(D const&) = default; // UndCC_Violation - T may not be copyable
        D& operator=(D const&) = default; // UndCC_Violation - T may not be copyable
        D(D&&) = default; // UndCC_Violation - T may not be movable
        D& operator=(D&&) = default; // UndCC_Violation - T may not be movable
