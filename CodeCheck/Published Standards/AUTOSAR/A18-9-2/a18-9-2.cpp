...
class A
{
  public:
    explicit A(std::string&& s)
            : str(std::move(s)) // UNDCC_Valid - forwarding rvalue reference
    {
    }
...
void Fn2(T&& param)
{
  Fn1(std::forward<T>(param)); // UNDCC_Valid - forwarding forwarding reference
}
template <typename T>
void Fn3(T&& param)
{
  Fn1(std::move(param)); // UNDCC_Violation - forwarding forwarding reference
  // via std::move
}
void Fn4() noexcept
{
