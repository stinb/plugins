...
#include <cstdint>
#include <limits>
class A // UndCC_Valid - A provides user-defined constructors, invariant and
// interface
{
...
struct B // UndCC_Violation - non-POD type defined as struct
{
public:
    static constexpr std::int32_t maxValue =
        std::numeric_limits<std::int32_t>::max();
...
struct C // UndCC_Valid - POD type defined as struct
{
    std::int32_t x;
    std::int32_t y;
};
class D // UndCC_Valid - POD type defined as class, but not compliant with
// M11-0-1
{
public:
    std::int32_t x;
