...
void FVeryBad() noexcept(false)
{
    ...
    if (i < 10)
    {
        throw(*e); // UndCC_Violation - fVeryBad() is not able to clean-up
        // allocated memory
    }
...
void FBad() noexcept(false)
{
    ...
    if (i < 10)
    {
        throw std::logic_error("Logic Error 2"); // UndCC_Violation - exits from
    ...
    {
        throw std::runtime_error("Runtime Error 3"); // UndCC_Violation - exits
        // from fBad() without
...
void FGood() noexcept(false)
{
    ...
    if (i < 10)
    {
        ...
        throw std::logic_error("Logic Error 4"); // UndCC_Valid - deleting y
    ...
    {
        delete y; // Deletes allocated resource before throwing an exception
        throw std::runtime_error("Runtime Error 5"); // UndCC_Valid - deleting y
        // variable before
    ...
    else if (i < 30)
    {
        ...
        throw std::invalid_argument(
                "Invalid Argument 1"); // UndCC_Valid - deleting
        // y variable before
        // exception leaves the
        // fGood() scope
    }
...
void FBest() noexcept(false)
{
    ...
    if (i < 10)
    {
        throw std::logic_error("Logic Error 6"); // UndCC_Valid - leaving the
        // fBest() scope causes
        // deallocation of all
    ...
    {
        throw std::runtime_error("Runtime Error 3"); // UndCC_Valid - leaving the
        // fBest() scope causes
        // deallocation of all
        // automatic variables,
        // unique_ptrs, too
    ...
    {
        throw std::invalid_argument(
                "Invalid Argument 2"); // UndCC_Valid - leaving the fBest() scope
...
void FBest2() noexcept(false)
{
    ...
    if (i < 10)
    {
        throw std::logic_error("Logic Error 7"); // UndCC_Valid - leaving the
    ...
    {
        throw std::runtime_error("Runtime Error 4"); // UndCC_Valid - leaving the
        // fBest2() scope causes
        // a1 variable
        // deallocation
        // automatically
    ...
    {
        throw std::invalid_argument(
                "Invalid Argument 3"); // UndCC_Valid - leaving the fBest2() scope
