...
public:
    ...
    {
        std::lock_guard<std::mutex>{m_mtx}; // UndCC_Violation: temporary object
    ...
    {
        std::lock_guard<std::mutex> guard{m_mtx};  // UndCC_Violation
        m_value = value;
    }
...
{
    // UndCC_Valid: Not only constructing a temporary object
