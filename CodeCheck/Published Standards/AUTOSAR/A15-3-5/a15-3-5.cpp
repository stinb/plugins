...
void Fn()
{
    ...
    catch (const std::logic_error &e) // UndCC_Valid - caught by const reference
    {
        // Handle exception
    }
    catch (std::runtime_error &e) // UndCC_Valid - caught by reference
    {
        ...
        std::runtime_error
            e) // UndCC_Violation - derived types will be caught as the base type
    {
        std::cout
            << e.what()
