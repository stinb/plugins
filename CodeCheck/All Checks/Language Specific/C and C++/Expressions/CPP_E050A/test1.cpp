...
{
  return (num / den);    /* UNDCC_VIOLATION Part A- value of den could be zero */
...
float Division_B(float num, float den)
{
  ...
  return (num / den);    /* UNDCC_Valid Part A- value of den checked before division */
...
int main()
{
  ...
  int result_C = 120 / 0;      /* VIOLATION in Part B */
  float bad_modulo = 30 % 0;   /* VIOLATION in Part B */
